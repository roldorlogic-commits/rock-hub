...
async function loadVStats() {
  try {
    ...
    const myTasks = tasks.filter(t => t.AssignedTo?.toLowerCase() === myEmail.toLowerCase() && t.Status !== 'Completed');
    ...
    <div class="event-row">
      ...
      <div class="event-info">
        ...
        <div class="event-meta">
          ...
          ${ev.StartTime ? `<span class="event-meta-sep">·</span><span>${ev.StartTime}</span>` : ''}
        </div>
      </div>
...
function renderVEventsPreview(events) {
  ...
  const upcoming = events.filter(e => e.EventName).slice(0, 4);
...
function renderVEventsFull(events) {
  ...
  el.innerHTML = events.length
    ? events.map(e => vEventRow(e, true)).join('')
...
async function loadVTasks() {
  try {
    ...
    const mine = tasks.filter(t => t.AssignedTo?.toLowerCase() === myEmail);
...
async function loadAnnouncements() {
  try {
    ...
    const el = document.getElementById('vAnnouncements');
    const active = items.filter(a => a.Title && (!a.Status || a.Status !== 'Archived'));
    el.innerHTML = active.length
      ? active.slice(0, 4).map(a => `
          <div class="announcement-item">
            <div class="announcement-title">${a.Pinned === 'TRUE' || a.Pinned === '1' ? '📌 ' : ''}${a.Title}</div>
          ...
          <div class="list-item">
            ...
            <div class="item-info">
              <div class="item-title">
                ${d.FileURL ? `<a href="${d.FileURL}" target="_blank" style="color:var(--text-white);">${d.Title}</a>` : d.Title}
              </div>
              <div class="item-sub">${d.Category || d.FileType || '—'}</div>
            </div>
          </div>`).join('')
      : emptyState('No public resources yet.');

    document.getElementById('vResources').innerHTML    = html;
    document.getElementById('vResourcesFull').innerHTML = pub.length
      ? pub.map(d => `<div class="list-item"><div class="file-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div><div class="item-info"><div class="item-title">${d.FileURL ? `<a href="${d.FileURL}" target="_blank" style="color:var(--text-white);">${d.Title}</a>` : d.Title}</div><div class="item-sub">${d.Category || '—'} · ${fmtDate(d.UploadDate)}</div></div></div>`).join('')
