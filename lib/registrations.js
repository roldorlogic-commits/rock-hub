...
// Returns null for placeholder/blank emails so that records sharing a
// placeholder are never collapsed onto each other.
async function findByEmail(eventId, emailAddr) {
  const needle = (emailAddr || '').toLowerCase();
  if (!needle || isPlaceholder(needle)) return null;
  const regs = await sheets.getEventRegistrations();
...
async function upsertVolunteerRegistrant({ eventId, firstName, lastName, email, phone, memberID }) {
  const existing = await findByEmail(eventId, email);
  if (existing) {
    const updates = {};
    if (existing.Source !== 'volunteer') { updates.Source = 'volunteer'; updates.Category = 'Volunteer'; }
    if (memberID && !existing.MemberID) updates.MemberID = memberID;
    if (Object.keys(updates).length) {
      return sheets.updateRowFields('EventRegistrations', 'RegistrationID', existing.RegistrationID, updates);
    }
    return existing;
  }
