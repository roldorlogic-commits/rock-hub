...
async function updateEvent(calEventId, { title, description, location, start, end, allDay }) {
  const cal = buildClient();
  const id  = calendarId();
  if (!id) throw new Error('GOOGLE_CALENDAR_ID is not set.');
  ...
  const res = await cal.events.update({ calendarId: id, eventId: calEventId, requestBody: body });
  const ev  = res.data;
  return {
    id:       ev.id,
    title:    ev.summary,
...
// Delete a calendar event. Silently succeeds if the event is already gone (410).
async function deleteEvent(calEventId) {
  const cal = buildClient();
  const id  = calendarId();
  if (!id) throw new Error('GOOGLE_CALENDAR_ID is not set.');
  try {
    await cal.events.delete({ calendarId: id, eventId: calEventId });
  } catch (err) {
    if (err?.code === 410 || err?.code === 404) return; // already deleted
    throw err;
  }
}
