...
async function createPosition(eventId, fields) {
  ...
  return sheets.appendRow('VolunteerPositions', {
    PositionID: `POS${Date.now()}`, EventID: eventId,
...
async function createSignup(eventId, posId, fields) {
  ...
  return sheets.appendRow('VolunteerSignups', {
    SignupID: `SIG${Date.now()}`, PositionID: posId, EventID: eventId,
...
module.exports = {
  ...
  getSignupsByPosition, getSignupsByEvent, createSignup, updateSignupStatus
};
