...
// Removes a signup entirely (board action — event-scoped only; contact is untouched).
// Recomputes slot counts on the owning position afterward.
async function deleteSignup(signupId) {
  const signup = await sheets.findRow('VolunteerSignups', 'SignupID', signupId);
  if (!signup) return false;
  const ok = await sheets.deleteRow('VolunteerSignups', 'SignupID', signupId);
  if (ok) await _syncPositionSlots(signup.PositionID);
  return ok;
}
