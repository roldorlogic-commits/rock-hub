...
// ── Generic letterhead document PDF ──────────────────────────────────────────
// Used by the AI agent to export drafted letters / reports.

function buildLetterheadHtml(title, bodyHtml) {
  const logoSrc = getLogoB64();
  return `<!DOCTYPE html>
<html lang="en">
<head>
...
* { margin: 0; padding: 0; box-sizing: border-box; }
html, body { background: #fff; color: #14203a; font-family: 'Lato', Georgia, serif; }
.ph { position: fixed; top: 0; left: 0; right: 0; z-index: 100; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
.ph-band { height: 62px; background-color: #14203a; display: flex; align-items: center; justify-content: space-between; padding: 0 36px; }
.ph-text { font-family: Georgia, serif; color: #fff; font-size: 11px; font-weight: bold; letter-spacing: 0.09em; text-transform: uppercase; line-height: 1.4; flex: 1; }
.ph-sub  { display: block; font-size: 7.5px; font-weight: normal; color: #C9A96E; letter-spacing: 0.18em; margin-top: 3px; }
.ph-logo { height: 46px; width: auto; display: block; flex-shrink: 0; }
.ph-rule { height: 3px; background-color: #C9A96E; }
.pf { position: fixed; bottom: 0; left: 0; right: 0; z-index: 100; height: 50px; background-color: #14203a; display: flex; align-items: center; justify-content: space-between; padding: 0 28px; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
.pf-logo { height: 30px; width: auto; }
.pf-text { color: #fff; font-family: Georgia, serif; font-size: 7px; letter-spacing: 0.05em; text-align: center; flex: 1; padding: 0 18px; line-height: 1.4; }
.wm { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%); width: 54%; opacity: 0.09; z-index: 0; pointer-events: none; }
.content { position: relative; z-index: 1; padding: 80px 50px 65px; }
.doc-title { font-family: 'Cinzel', Georgia, serif; font-size: 20px; font-weight: 700; color: #14203a; margin-bottom: 6px; }
.doc-date  { font-size: 10px; color: #5a6474; letter-spacing: 0.05em; margin-bottom: 28px; border-bottom: 1.5px solid #ddd3bf; padding-bottom: 14px; }
.doc-body h2 { font-family: 'Cinzel', Georgia, serif; font-size: 11px; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: #C9A96E; margin: 22px 0 8px; }
.doc-body p  { font-size: 12px; line-height: 1.8; margin-bottom: 12px; }
.doc-body ul, .doc-body ol { font-size: 12px; line-height: 1.8; margin: 0 0 12px 20px; }
...
<div class="ph">
  <div class="ph-band">
    <div class="ph-text">The ROCK Association<span class="ph-sub">Recruiting &nbsp;&middot;&nbsp; Training &nbsp;&middot;&nbsp; Empowering</span></div>
    <img class="ph-logo" src="${logoSrc}" alt="ROCK">
  </div>
  <div class="ph-rule"></div>
</div>
<div class="pf">
  <img class="pf-logo" src="${logoSrc}" alt="ROCK">
  <div class="pf-text">The Recruiters Of Christ&rsquo;s Kingdom Association &nbsp;&middot;&nbsp; gorock.org</div>
</div>
<img class="wm" src="${logoSrc}" alt="">
<div class="content">
...
async function generateLetterheadPdf(title, bodyHtml) {
  const html    = buildLetterheadHtml(title, bodyHtml);
  const browser = await getBrowser();
  const page    = await browser.newPage();
  try {
    await page.setContent(html, { waitUntil: 'load', timeout: 15000 });
...
module.exports = { generateItineraryPdf, generateVolunteerRosterPdf, generateLetterheadPdf };
